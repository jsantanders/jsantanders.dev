...
interface TweetProps {
	...
	author: {
		...
		username: string;
		verified: boolean;
		profile_image_url: string; // Assuming this is for the profile image
	};
	media?: {
		media_key: string; // Unique key for each media item
		height: number;
		width: number;
		url: string;
	}[]; // Array of media objects, optional
	created_at: string; // Assuming this is an ISO date string
	public_metrics: {
		retweet_count: number;
		reply_count: number;
		like_count: number;
		quote_count: number; // Although not used directly, it's part of public_metrics
	};
	referenced_tweets?: {
		type: string; // 'quoted', 'retweeted', or 'replied_to'
		id: string;
		author: {
			name: string;
			username: string;
	...
	}[]; // Array of referenced tweets, optional
}
...
export const Tweet: React.FC<TweetProps> = ({
	...
	return (
		<div className="tweet my-4 w-full rounded-lg border border-gray-200 bg-white px-6 py-4 dark:border-gray-800 dark:bg-gray-900">
			...
			<div className="mb-1 mt-4 whitespace-pre-wrap text-lg leading-normal !text-gray-700 dark:!text-gray-200">
				{formattedText}
			</div>
			{media?.length ? (
				<div
